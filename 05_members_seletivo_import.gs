...
function members_importFromSeletivoResults() {
  ...
  const pendentes = members_findAvaliacaoRowsPendentesImport_();
  Logger.log("members_importFromSeletivoResults: pendentes=" + pendentes.length);

  const futureSheet = members_sheetByKey_(SETTINGS.futureKey);
  if (!futureSheet) {
    throw new Error("Não foi possível localizar MEMBERS_FUTURO.");
  }

  pendentes.forEach(av => {
    const rga = String(av["RGA"] || "").trim();
    const email = members_normalizeEmail_(av["Email"]);
    const resultado = String(av["Resultado"] || "").trim();

    const insc = members_findInscricaoByRgaOrEmail_(rga, email);
    if (!insc) {
      Logger.log("Sem inscrição encontrada para RGA=" + rga + " | Email=" + email);
      return;
    }
    ...
    if (members_futureHasRgaOrEmail_(rga, emailPrincipal)) {
      Logger.log("Candidato já existe em MEMBERS_FUTURO. RGA=" + rga + " | Email=" + emailPrincipal);
    ...
    const payload = members_buildFutureRowFromInscricao_(insc, processStatus);
        members_appendObjectByHeaders_(futureSheet, payload);

        const newRow = futureSheet.getLastRow();
        const futureHeaders = futureSheet.getRange(1, 1, 1, futureSheet.getLastColumn()).getValues()[0]
        .map(h => String(h || "").trim());
    ...
    Logger.log(
      "Importado do seletivo para MEMBERS_FUTURO: " +
