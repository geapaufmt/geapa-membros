...
function normative(minimum = 6, presentationBlock = 4, suffix = 'A') {
  const item = (id, value) => ({
    parametroId: id,
    valor: value,
    unidade: 'DIAS',
...
function baseRequest(overrides = {}, parameters = normative()) {
  const snapshot = sandbox.membersVinculoSnapshotParameters_(parameters, new Date(2026, 2, 10, 12));
...
test('66 desligamento final sem ata bloqueia', () => code(() => sandbox.membersVinculoAssertFinalDecisionDocument_({ TIPO_SOLICITACAO: 'DESLIGAMENTO_VOLUNTARIO' }, {}), 'ATA_DECISAO_FINAL_OBRIGATORIA'));
